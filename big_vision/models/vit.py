...
class Encoder1DBlock(nn.Module):
  ...
  @nn.compact
  def __call__(self, x, cond=None, deterministic=True):
    if cond is not None and self.adaln:
      cond = nn.Dense(6 * x.shape[-1], kernel_init=nn.initializers.zeros, dtype=self.dtype_mm)(cond)
      shift_0, scale_0, mask_0, shift_1, scale_1, mask_1 = jnp.split(cond, 6, axis=-1)
    elif cond is not None and not self.adaln:
      x = jnp.concatenate([cond[:, None, :], x], axis=1)

    out = {}
    x = nn.with_logical_constraint(x, ("act_batch", "act_len", "act_emb"))
    y = nn.LayerNorm()(x)
    if cond is not None and self.adaln:
      y = modulate(y, shift_0, scale_0)
    ...
    if cond is not None and not self.adaln:
      x = x[:, 1:]
    return x, out
  ...
  def __call__(self, x, cond=None, deterministic=True):
    ...
    if self.scan:
      ...
      for lyr in range(self.depth):
        ...
        x, out[f"block{lyr:02d}"] = block_cur(x,
                                              cond=cond, 
                                              deterministic=deterministic)
...
def ViT(num_classes=None, *, variant=None, **kw):  # pylint: disable=invalid-name
  """Factory function, because linen really don't like what I'm doing!"""
  return _ViT(num_classes, **{**decode_variant(variant), **kw})
