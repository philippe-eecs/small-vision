...
def gather_metrics(metrics):
  if jax.process_count() == 1:
    ...
    gather = jax.experimental.multihost_utils.process_allgather(metrics)
    gather = jax.device_get(gather)

    def process(x):
      if x.ndim == 5:
        P, B, H, W, C = x.shape
        return x.reshape(P * B, H, W, C)
      elif x.ndim == 1:
        return jnp.mean(x, axis=0)
      else:
        return x
    return jax.tree_map(process, gather)


# pylint: disable=logging-fstring-interpolation
def make_grid(samples, num_samples=36, padding=2, pad_value=0.0, unnormalize=False):
  '''
  Makes a grid of images from a batch of images.
  '''
  assert samples.ndim == 4

  ndarray = samples[:num_samples].copy() # take only what we need
  del samples # free memory

  if unnormalize or ndarray.dtype == np.float32:
    #assert ndarray.min() >= -1.0 and ndarray.max() <= 1.0 #assumes -1, 1
    ndarray = np.clip(ndarray, -1.0, 1.0)
    ndarray = ndarray * 0.5 + 0.5 #brings -1, 1 to 0, 1
  ...
  if not (isinstance(ndarray, np.ndarray) or
          (isinstance(ndarray, list) and
           all(isinstance(t, np.ndarray) for t in ndarray))):
...
def save_images_to_gcs(images, workdir, folder, name):
    fid_bytes = images.dumps()

    # Write the data to GCS using tensorflow.io.gfile
    with gfile(f'{workdir}{folder}/{name}', 'wb') as f:
        f.write(fid_bytes)
