...
def training(input_config, prefetch=2, skip_decode=("image",)):
  ...
  if isinstance(input_config.data.get("name"), str):
    train_data = ds_core.get(**input_config.data, skip_decode=skip_decode)
...
def make_for_inference(
    ...
    cache_raw=False, cache_final=False, repeats=1):
  """Makes an input pipeline for inference.
  Repeats to add more data augmentation
  """
  ...
  num_batches = math.ceil(max(num_ex_per_process) * repeats / local_batch_size)
