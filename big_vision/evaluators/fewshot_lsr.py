...
class Evaluator:
  ...
  def _get_dataset(self, dataset_train, dataset_val, train_split, test_split):
    """Lazy-loads given dataset."""
    key = (dataset_train, dataset_val, train_split, test_split)
    try:
      return self._datasets[key]
    except KeyError:
      # NOTE: only supporting TFDS data for now for bwd compat/lazyness.
      train_data = ds_core.get(name=dataset_train, split=train_split)
      ...
      test_data = ds_core.get(name=dataset_val, split=test_split)
  ...
  def compute_fewshot_metrics(self, train_state, seed,
                              dataset_train, dataset_val, train_split, test_split):
    ...
    self._repr[dataset_train] = (repr_train, labels_train,
    ...
    for shots in self.shots:
      ...
      acc = _eig_fewshot_acc_fn(
          cache, repr_test, labels_test, self.l2_reg)
